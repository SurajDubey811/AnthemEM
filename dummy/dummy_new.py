...
for ele in root.findall(".//ns5:benefitComponent", namespaces=namespaces):
    ...
    if (ele.findall(".//ns5:calculationSituation", namespaces=namespaces)) != 0:
        for calc_sit in ele.iterfind(".//ns5:calculationSituation", namespaces=namespaces):
            ...
            if (calc_sit.findall(".//ns5:calculationStep", namespaces=namespaces)) != 0:
                for item in calc_sit.findall(".//ns5:calculationStep", namespaces=namespaces):
                    ...
                    if item.find(".//ns5:benefitType", namespaces=namespaces).get("value") == "Coinsurance":
                        ...
                        if item.find(".//ns5:claimsAccumulatorName", namespaces=namespaces) is not None:
                            row_dict['COIN_ACCUM'] = item.find(".//ns5:claimsAccumulatorName",
                                                               namespaces=namespaces).get(
                                'value')
                        else:
                            row_dict['COIN_ACCUM'] = None

                        row_dict['COIN_ACCUM_NET'] = item.find(".//ns5:accumNetworkType", namespaces=namespaces).get(
                            'value')
                    ...
                    if item.find(".//ns5:benefitType", namespaces=namespaces).get("value") == "Copayment":
                        ...
                        if item.find(".//ns5:claimsAccumulatorName", namespaces=namespaces) is not None:
                            row_dict['COPAY_ACCUM'] = item.find(".//ns5:claimsAccumulatorName",
                                                                namespaces=namespaces).get(
                                'value')
                        else:
                            row_dict['COPAY_ACCUM'] = None

                        row_dict['COPAY_ACCUM_NET'] = item.find(".//ns5:accumNetworkType", namespaces=namespaces).get(
                            'value')
                    ...
                    if item.get("name") == "DedINNT1IndMed":
                        ...
                        if item.find(".//ns5:claimsAccumulatorName", namespaces=namespaces) is not None:
                            row_dict['DED_ACCUM'] = item.find(".//ns5:claimsAccumulatorName",
                                                              namespaces=namespaces).get(
                                'value')
                        else:
                            row_dict['DED_ACCUM'] = None

                        row_dict['DED_ACCUM_NET'] = item.find(".//ns5:accumNetworkType", namespaces=namespaces).get(
                            'value')
                    ...
                    if item.get("name") == "DedINNT1FamMed":
                        ...
                        row_dict["DED_AGG_PERIOD"] = item.find(".//ns5:benefitPeriod", namespaces=namespaces).get(
                            'value')
                        row_dict["DED_AGG_EOB"] = int(item.find(".//ns5:eobCode", namespaces=namespaces).get('value'))
                        if item.find(".//ns5:claimsAccumulatorName", namespaces=namespaces) is not None:
                            row_dict['DED_AGG_ACCUM'] = item.find(".//ns5:claimsAccumulatorName",
                                                                  namespaces=namespaces).get(
                                'value')
                        else:
                            row_dict['DED_AGG_ACCUM'] = None

                        row_dict['DED_AGG_ACCUM_NET'] = item.find(".//ns5:accumNetworkType", namespaces=namespaces).get(
                            'value')
                    ...
                    if item.find(".//ns5:benefitType", namespaces=namespaces).get("value") == "DollarLimit":
                        ...
                        row_dict["DOL_LMT_PERIOD"] = item.find(".//ns5:benefitPeriod", namespaces=namespaces).get(
                            'value')
                        row_dict["DOL_LMT_EOB"] = int(item.find(".//ns5:eobCode", namespaces=namespaces).get('value'))
                        if item.find(".//ns5:claimsAccumulatorName", namespaces=namespaces) is not None:
                            row_dict['DOL_LMT_ACCUM'] = item.find(".//ns5:claimsAccumulatorName",
                                                                  namespaces=namespaces).get(
                                'value')
                        else:
                            row_dict['DOL_LMT_ACCUM'] = None

                        row_dict['DOL_LMT_ACCUM_NET'] = item.find(".//ns5:accumNetworkType", namespaces=namespaces).get(
                            'value')
                    ...
                    if item.find(".//ns5:benefitType", namespaces=namespaces).get("value") == "Unit":
                        ...
                        if item.find(".//ns5:claimsAccumulatorName", namespaces=namespaces) is not None:
                            row_dict['UNIT_ACCUM'] = item.find(".//ns5:claimsAccumulatorName",
                                                               namespaces=namespaces).get(
                                'value')
                        else:
                            row_dict['UNIT_ACCUM'] = None

                        row_dict['UNIT_ACCUM_NET'] = item.find(".//ns5:accumNetworkType", namespaces=namespaces).get(
                            'value')
...
df_final = df_result.drop_duplicates()
df_final.to_excel("xml_data.xlsx")
...
df_xl = pd.read_excel("SAM_ARTIFACTS/common_report/duplicate_nodes_common_report.xlsx")

df_sd = df_xl.drop_duplicates()
print(len(df_sd))
print(summ)