...
def duplicate_identification_data(excel_file):
    ...
    df = excelData[['id', 'BEN_UID', "SIT_UID", 'PATH_BEN_COMP_NAME', 'HASH', 'SIT_CHANGE', 'BEN_CHANGE']]
...
def get_duplication_verdict(child_hash, parent_hash):
    A = set(child_hash)
    B = set(parent_hash)
...
def main():
    xl_data = duplicate_identification_data('SAM_ARTIFACTS/common_report'
                                            '/31KT_Plan_Normalized_duplication_common_report.xlsx')
    xl_data = xl_data[xl_data['SIT_CHANGE'] != 'Deletion']
    required_columns = ["BEN_UID", "SIT_UID", "PATH_BEN_COMP_NAME", "HASH"]
    required_df = xl_data[required_columns]
    required_df['BEN_COMP_LVL'] = required_df[['PATH_BEN_COMP_NAME']]. \
        apply(lambda a: len(str(a["PATH_BEN_COMP_NAME"]).split(" > ")), axis=1)
    ...
    final_df = required_df.groupby(["PATH_BEN_COMP_NAME", "BEN_COMP_LVL", "PATH_ID", "PARENT_ID"])[
        ["BEN_UID", "SIT_UID", "PATH_BEN_COMP_NAME", "BEN_COMP_LVL", "HASH", "PATH_ID"]].apply(
        lambda g: g["HASH"].tolist()).reset_index(name='LIST_HASH_VAL')
    ...
    #                                         path_id_dict), axis=1)
    final_df = final_df.sort_values(by='BEN_COMP_LVL', ascending=False)

    # final_df["Verdict"] = final_df[["LIST_HASH_VAL", "PARENT_HASH_LIST"]]. \
    #     apply(lambda a: "Yes" if get_duplication_verdict(a["LIST_HASH_VAL"], "PARENT_HASH_LIST") else "No", axis=1)
    final_df.to_excel("parent_child.xlsx")
    final_df["BEN_COMP_TO_DELETE"] = final_df[["LIST_HASH_VAL", "PARENT_HASH_LIST"]]. \
        apply(lambda a: "Yes" if (set(a["LIST_HASH_VAL"]).issubset(set(a["PARENT_HASH_LIST"]))) else "No", axis=1)

    final_df.to_excel("final_verdict.xlsx")
    # deletion_dict = dict()
    #
