...
class HTMLReporter(object):
    ...
    def pytest_runtest_teardown(self, item, nextitem):
        global _test_name, _duration, _test_desc, _test_id
        _test_name = item.name
        _test_desc = str(item.function.__doc__)
        _test_id = _test_desc.split()[0]
        _test_desc = " ".join(item for item in _test_desc.split()[1:])
        # _test_desc = item.error
    ...
    def report_path(self):
        if '.html' in self.path:
            ...
            HTMLReporter.base_path = os.path.abspath(logfile)
            return os.path.abspath(logfile), 'AnthemEM Test Report.html'
        ...
        if rep.failed:
            if getattr(rep, "when", None) == "call":
                if hasattr(rep, "wasxfail"):
                    ...
                    if rep.longrepr:
                        longerr = ""
                        if "AssertionError" in rep.longreprtext:
                            error_msg = "AssertionError: " + str(rep.longreprtext)
    ...
    def append_test_metrics_row(self):
        ...
        self.json_data['content']['suites'].setdefault(len(_test_suite_name), {}).setdefault('tests', {}).setdefault(
            len(_scenario) - 1, {})['test_id'] = str(_test_id)
        self.json_data['content']['suites'].setdefault(len(_test_suite_name), {}).setdefault('tests', {}).setdefault(
            len(_scenario) - 1, {})['test_desc'] = str(_test_desc)
    ...
    def append_suite_metrics_row(self, name):
        ...
        suite_row_text = suite_row_text.replace("__suite_name__", "AnthemEM Test Suite")
